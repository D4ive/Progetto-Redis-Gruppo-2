...
def registra_utente(ruolo):
    ...
    if db.utente_esiste(username, ruolo):
    ...
    if db.crea_utente(username, ruolo, email, password):
...
def login_utente():
    """Gestisce il processo di login con tentativi limitati"""
    max_tentativi = 3
    ...
    for tentativo in range(max_tentativi):
        ...
        ruolo = db.ottieni_ruolo_utente(username)
    ...
    for tentativo in range(max_tentativi):
        ...
        if db.verifica_password(username, ruolo, password):
...
def menu_principale(ruolo):
    """Menu principale per autenticazione"""
    while True:
        try:
